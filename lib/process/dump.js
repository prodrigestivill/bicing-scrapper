...
const dump_csv = async (model, file) => {
  const raw = db.seq.getDialect() !== 'sqlite';
  let data = await model.findAll({ raw });
...
const dump_stations = async (p) => {
  const raw = db.seq.getDialect() !== 'sqlite';
  let stations = await station.findAll({ raw });
...
module.exports = async () => {
  switch (config.save_data_as) {
    case 'csv':
      ...
      await dump_csv(station, path.join(p, 'stations.csv'));
      await dump_csv(station_log, path.join(p, 'stations_log.csv'));
      break;
    case 'sql':
      const p2 = path.join(config.out_path, 'sql_dump');
      await fs_mkdir(p2, { recursive: true });
      await dump_stations(p2);
      await dump_csv(station_log, path.join(p2, 'stations_log.csv'));
      break;
    default:
