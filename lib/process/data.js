...
const model = require('../utils/db').models.data;
...
module.exports = async (stations, data, updated) => {
  let p, insert;
  if (constants.save_data_as !== 'sql') {
    p = path.join(constants.out_path, `${updated.toISOString().substring(0, 7)}`);
    await fs_mkdir(p, { recursive: true });
  }
  switch (constants.save_data_as) {
    case 'csv':
  ...
  for (const obj of data) {
    ...
    try {
      ...
      console.log('Error processing station data:', obj);
    }
  }
};
