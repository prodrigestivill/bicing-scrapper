...
const self = module.exports = {
  ...
  csv_opts: { // CSV Options from: https://csv.js.org/stringify/options/
    ...
    rowDelimiter: 'unix',
    formatters: {
      boolean: (val) => self.csv_opts.quotedString
        ? (val ? '"t"' : '"f"') : (val ? 't' : 'f'),
      date: (val) => self.csv_opts.quotedString
        ? `"${val.toISOString()}"` : val.toISOString(), // "YYYY-MM-DDTHH:mm:ss.sssZ"
      object: (val) => Array.isArray(val)
        ? (self.csv_opts.quotedString ? `"${val.join(',')}"` : val.join(','))
        : JSON.stringify(val)
    },
